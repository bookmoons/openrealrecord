  ...
  txConfirmed (
    ...
    return new Promise(function executeTxConfirmed (resolve, reject) {
      ...
      function handleTimedOut () {
        console.log('handleTimedOut')
        walletClient.removeListener('confirmed', handleTxConfirmed)
        reject(new Error('timed out'))
      }

      function handleTxConfirmed (walletId, tx) {
        console.log('handleTxConfirmed')
        checkTargetTxConfirmed(tx)
      }

      function checkTargetTxConfirmed (tx) {
        console.log(tx)
        if (tx.hash !== txid) return
      ...
      walletClient.client.bind('confirmed', handleTxConfirmed)
      console.log('watching new confirmations')

      // Detect already confirmed
      console.log('detecting already confirmed')
      walletClient.getTX(txid).then(checkTargetTxConfirmed)
    })
  }
}
